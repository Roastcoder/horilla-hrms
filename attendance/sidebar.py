...
SUBMENUS = [
    ...
    {
        "menu": _("Attendances "),
        "redirect": reverse("call_attendance:dashboard"),
        "accessibility": "attendance.sidebar.call_attendance_accessibility",
    }, 
    {
        "menu": _("Attendances View"),
...
def call_attendance_accessibility(request, submenu, user_perms, *args, **kwargs):
    """
    Check if the user has permission to view call attendance.
    """
    return request.user.has_perm("attendance.view_callattendance") or is_reportingmanager(
        request.user
    )
