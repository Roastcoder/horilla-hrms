...
def attendance_view(request):
    """
    This method is used to view attendances including call attendance data.
    """
    ...
    # Call attendance data
    call_attendances = CallAttendance.objects.filter(
        employee_id__is_active=True
    ).select_related('employee_id', 'updated_by').order_by('-attendance_date')
    ...
    # Filter call attendances for subordinates
    call_attendances = filtersubordinates(
        request, call_attendances, "attendance.view_attendance"
    )
    
    check_attendance = Attendance.objects.all()
    check_call_attendance = CallAttendance.objects.all()
    
    if check_attendance.exists() or check_call_attendance.exists():
    ...
    call_attendances_ids = json.dumps(
        [
            instance.id
            for instance in paginator_qry(
                call_attendances, request.GET.get("cpage")
            ).object_list
        ]
    )
    ...
    return render(
        ...
        template,
        {
            ...
            "attendances_ids": attendances_ids,
            "call_attendances_ids": call_attendances_ids,
            "call_attendances": paginator_qry(call_attendances, request.GET.get("cpage")),
            "f": filter_obj,
            "pd": previous_data,
...
def work_records_change_month(request):
    ...
    # Get sales incentive data
    from sales_incentive.models import Lead, LoanType
    
    # Calculate loan and attendance points for each employee
    employee_data = {}
    for employee in employees:
        # Get disbursed loans for the month
        disbursed_loans = Lead.objects.filter(
            employee=employee,
            status='DISBURSED',
            disbursed_date__year=year,
            disbursed_date__month=month
        ).select_related('loan_type')
        
        # Calculate loan points
        total_loan_amount = sum(loan.loan_amount_lac for loan in disbursed_loans)
        total_loan_points = sum(loan.loan_amount_lac * loan.loan_type.points_per_lac for loan in disbursed_loans)
        
        # Calculate attendance points (present days * 10 points)
        present_days = sum(1 for current_date in month_dates 
                          if work_records_dict.get((employee.id, current_date)) 
                          and work_records_dict.get((employee.id, current_date)).work_record_type == 'FDP')
        attendance_points = present_days * 10
        
        employee_data[employee.id] = {
            'total_loan_amount': total_loan_amount,
            'total_loan_points': total_loan_points,
...
def work_record_export(request):
    ...
    for employee in employees:
        # Calculate loan and attendance data
        disbursed_loans = Lead.objects.filter(
            employee=employee,
            status='DISBURSED',
            disbursed_date__year=year,
        ...
        present_days = sum(1 for day in all_date_objects 
                          if record_lookup.get((employee, day)) == 'FDP')
        attendance_points = present_days * 10
        
        row_data = {
            "Employee": str(employee),
            "Total Loan Amount (Lac)": f"{total_loan_amount:.2f}",
            "Total Loan Points": f"{total_loan_points:.2f}",
            "Present Days": present_days,
            "Attendance Points": attendance_points,
            "Total Points": f"{total_loan_points + attendance_points:.2f}"
        }
    ...
    # Add loan details for each employee
    for employee in employees:
        disbursed_loans = Lead.objects.filter(
            employee=employee,
            status='DISBURSED',
        ...
        for i, loan in enumerate(disbursed_loans):
            loan_row = {
                "Employee": f"{employee} - Loan {i+1}",
                "Loan Type": loan.loan_type.name,
                "Loan Amount (Lac)": f"{loan.loan_amount_lac:.2f}",
                "Points per Lac": f"{loan.loan_type.points_per_lac:.2f}",
                "Loan Points": f"{loan.loan_amount_lac * loan.loan_type.points_per_lac:.2f}",
                "Disbursed Date": loan.disbursed_date.strftime('%Y-%m-%d') if loan.disbursed_date else ""
            }
            # Fill remaining columns with empty values
            for formatted_day in formatted_dates:
                loan_row[formatted_day] = ""
            data_rows.append(loan_row)

    columns = ["Employee", "Total Loan Amount (Lac)", "Total Loan Points", "Present Days", "Attendance Points", "Total Points"] + formatted_dates
    df = pd.DataFrame(data_rows, columns=columns)

    output = io.BytesIO()
    with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
        df.to_excel(writer, index=False, sheet_name="Work Records")
        workbook = writer.book
        worksheet = writer.sheets["Work Records"]

        # Format attendance columns
        formats = {
        ...
        # Apply formatting to attendance data
        for row_idx, row in enumerate(df.itertuples(index=False), start=1):
            for col_idx, cell_value in enumerate(row[6:], start=6):  # Start from attendance columns
    ...
    response["Content-Disposition"] = f'attachment; filename="work_record_export_{year}_{month:02d}.xlsx"'
    return response
