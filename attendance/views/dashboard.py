...
def dashboard(request):
    """
    This method is used to render individual dashboard for attendance module based on call attendance
    """
    ...
    # Call attendance statistics for today
    call_attendances_today = CallAttendance.objects.filter(attendance_date=today)
    call_present = call_attendances_today.filter(attendance_status='PRESENT').count()
    call_half_day = call_attendances_today.filter(attendance_status='HALF_DAY').count()
    call_absent = call_attendances_today.filter(attendance_status='ABSENT').count()
    call_manual = call_attendances_today.filter(source='MANUAL').count()
    
    # Calculate attendance ratio based on call attendance
    total_call_attendance = call_present + call_half_day + call_absent
    call_attendance_ratio = 0
    if total_employees > 0:
        call_attendance_ratio = f"{(total_call_attendance / total_employees) * 100:.2f}"

    return render(
        request,
        "attendance/dashboard/dashboard.html",
        {
            "marked_attendances_ratio": call_attendance_ratio,
            "on_time": call_present,
            "late_come": call_absent,
            "expected_attendances": total_employees,
            "marked_attendances": total_call_attendance,
            "call_present": call_present,
            "call_half_day": call_half_day,
            "call_absent": call_absent,
