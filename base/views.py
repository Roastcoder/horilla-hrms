...
def employee_permission_search(request, codename=None, uid=None):
    ...
    context["no_permission_models"] = NO_PERMISSION_MODALS
    context["employees"] = paginator_qry(employees, request.GET.get("page"))
    context["pd"] = request.GET.urlencode()
    return render(
        request,
        template,
        context,
...
def update_permission(request):
    """
    This method is used to update user permissions.
    """
    try:
        ...
        if not employee_id:
            return JsonResponse({"message": "Employee ID is required", "type": "danger"})
        
        employee = Employee.objects.filter(id=employee_id).first()
        if not employee:
            return JsonResponse({"message": "Employee not found", "type": "danger"})
        ...
        if permissions:
            ...
            return JsonResponse({
                "message": f"Updated permissions for {employee}", 
                "type": "success"
            })
        else:
            return JsonResponse({
                "message": f"All permissions cleared for {employee}", 
                "type": "info"
            })
...
def permission_table(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            form.save()
            messages.success(request, _("Employee permissions assigned successfully."))
            return HttpResponse("<script>window.location.reload()</script>")
...
def bulk_permission_update(request):
    """
    This method is used to update permissions for multiple employees at once.
    """
    try:
        employee_ids = request.POST.getlist("employee_ids")
        permissions = request.POST.getlist("permissions")
        action = request.POST.get("action", "assign")  # assign or clear
        
        if not employee_ids:
            return JsonResponse({"message": "No employees selected", "type": "danger"})
        
        employees = Employee.objects.filter(id__in=employee_ids, is_active=True)
        if not employees.exists():
            return JsonResponse({"message": "No valid employees found", "type": "danger"})
        ...
        for employee in employees:
            ...
            if action == "clear":
                user.user_permissions.clear()
                updated_count += 1
            elif action == "assign" and permissions:
                permission_objects = Permission.objects.filter(codename__in=permissions)
                user.user_permissions.set(permission_objects)
                updated_count += 1
        
        if updated_count > 0:
            action_text = "cleared" if action == "clear" else "updated"
            return JsonResponse({
                "message": f"Permissions {action_text} for {updated_count} employees",
                "type": "success"
            })
        else:
            return JsonResponse({"message": "No permissions were updated", "type": "info"})
...
@login_required
@require_http_methods(["POST"])
def location_checkin(request):
    """
    Handle check-in with exact location capture for non-caller attendance
    """
    try:
        ...
        if not latitude or not longitude:
            return JsonResponse({"message": "Location is required", "type": "danger"})
        ...
        if apps.is_installed("attendance"):
            ...
            if existing_attendance:
                return JsonResponse({"message": "Already checked in today", "type": "info"})
            
            # Create new attendance record
            attendance = Attendance.objects.create(
                employee_id=employee,
                attendance_date=today,
                attendance_clock_in=timezone.now().time(),
                attendance_clock_in_date=timezone.now(),
            ...
            return JsonResponse({
                "message": f"Checked in successfully at {address}",
                "type": "success",
                "attendance_id": attendance.id
            })
        else:
            return JsonResponse({"message": "Attendance module not available", "type": "danger"})
            
    except Exception as e:
        return JsonResponse({"message": f"Check-in failed: {str(e)}", "type": "danger"})
...
def location_checkout(request):
    ...
    try:
        ...
        if apps.is_installed("attendance"):
            ...
            if not attendance:
                return JsonResponse({"message": "No check-in record found", "type": "danger"})
            
            # Update with check-out info
            attendance.attendance_clock_out = timezone.now().time()
            attendance.attendance_clock_out_date = timezone.now()
            attendance.checkout_latitude = float(latitude) if latitude else None
            attendance.checkout_longitude = float(longitude) if longitude else None
            attendance.checkout_address = address
            attendance.save()
            
            return JsonResponse({
                "message": f"Checked out successfully at {address}",
                "type": "success"
            })
        else:
            return JsonResponse({"message": "Attendance module not available", "type": "danger"})
        ...
        return JsonResponse({"message": f"Check-out failed: {str(e)}", "type": "danger"})


@login_required
@permission_required("base.change_company")
def white_label_config(request):
    """
    Configure white labeling settings
    """
    if request.method == "POST":
        enable_white_label = request.POST.get("enable_white_label") == "on"
        
        # Update horilla_apps setting
        from horilla import horilla_apps
        horilla_apps.WHITE_LABELLING = enable_white_label
        
        # Save to settings file or database as needed
        messages.success(
            request,
            _(f"White labeling {'enabled' if enable_white_label else 'disabled'} successfully.")
        )
    ...
    from horilla import horilla_apps
    context = {
        "white_label_enabled": getattr(horilla_apps, "WHITE_LABELLING", False)
    }
    
    return render(request, "base/white_label_config.html", context)
